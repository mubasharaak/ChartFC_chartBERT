...
import configs.config_vit as CONFIG
from utils_data_vit import build_dataloaders

parser = argparse.ArgumentParser()
parser.add_argument('--data_root', default='data', type=str)
parser.add_argument('--lr', type=str)
args = parser.parse_args()
EXPT_DIR = os.path.join(args.data_root, 'experiments', "ChartFC_vit")
...
def train_epoch(model, train_loader, criterion, optimizer, epoch, config, val_loaders, test_loaders):
    ...
    for batch_idx, (txt, label, img, qid, ql, ocr, ocrl) in enumerate(train_loader):
        ...
        if batch_idx % save_steps == 0 and batch_idx != 0:
            ...
            curr_epoch_path = os.path.join(EXPT_DIR, str(epoch + 1) + '_' + str(save_steps) + '.pth')
            latest_path = os.path.join(EXPT_DIR, 'latest_' + str(save_steps) + '.pth')
